...
validParams<ComputeLinearElasticPFFractureStress>()
{
  ...
  MooseEnum Decomposition("strain_spectral strain_vol_dev stress_spectral strain_vol_dev_spectral none", "none");
...
{
  if ((_decomposition_type == Decomposition_type::strain_vol_dev || _decomposition_type == Decomposition_type::strain_vol_dev_spectral ||
...
void
ComputeLinearElasticPFFractureStress::computeStrainVolDevSpectral(Real & F_pos, Real & F_neg)
{
  // Isotropic elasticity is assumed and should be enforced
  const Real lambda = _elasticity_tensor[_qp](0, 0, 1, 1);
  const Real mu = _elasticity_tensor[_qp](0, 1, 0, 1);
...
//***do eigendecomp of strain0dev
  // Compute eigenvectors and eigenvalues of mechanical strain and projection tensor
  RankTwoTensor eigvec;
  std::vector<Real> eigval(LIBMESH_DIM);
  RankFourTensor SDpos = strain0dev.positiveProjectionEigenDecomposition(eigval, eigvec);
  RankTwoTensor strain0devpos = SDpos * strain0dev;
  RankTwoTensor strain0devneg = strain0dev - strain0devpos;
//***resume normal operations

  strain0tr_pos = strain0tr - strain0tr_neg;
  stress0neg = k * strain0tr_neg * I2 + 2.0*mu * strain0devneg;
  stress0pos = _elasticity_tensor[_qp] * _mechanical_strain[_qp] - stress0neg;
  // Energy with positive principal strains

  RankTwoTensor strain0dev2pos = strain0devpos * strain0devpos;
  RankTwoTensor strain0dev2neg = strain0devneg * strain0devneg;
  F_pos = 0.5 * k * strain0tr_pos * strain0tr_pos + mu * strain0dev2pos.trace();
  F_neg = 0.5 * k * strain0tr_neg * strain0tr_neg + mu * strain0dev2neg.trace();
...
ComputeLinearElasticPFFractureStress::computeQpStress()
{
  ...
  switch (_decomposition_type)
  {
    ...
    case Decomposition_type::stress_spectral:
      ...
      break;
    case Decomposition_type::strain_vol_dev_spectral:
      computeStrainVolDevSpectral(F_pos, F_neg);
      break;
    default:
    {
